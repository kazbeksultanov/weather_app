import 'package:drivers/ui/widgets/json_viewer.dart';
...
class _AliceCallResponseWidgetState
  ...
  List<Widget> _buildTextBodyRows() {
    ...
    final bodyContent = formatBody(_call.response!.body, getContentType(_call.response!.headers));
    if (_call.response!.body is Map<String, dynamic>) {
      return [
        const SizedBox(height: 16),
        const Text('Body:', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        JsonViewerCustom(_call.response!.body, arrowSize: 16),
        const SizedBox(height: 16),
      ];
    } else {
  ...
  List<Widget> _buildVideoBodyRows() {
    final List<Widget> rows = [];
    rows.add(getListRow('Body:', 'Video is not supported ${_call.uri}'));
    return rows;
