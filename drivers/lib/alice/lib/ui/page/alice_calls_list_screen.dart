...
import 'alice_time_stats_screen.dart';
...
class _AliceCallsListScreenState extends State<AliceCallsListScreen> {
  ...
  _AliceCallsListScreenState() {
    ...
    _menuItems.add(AliceMenuItem('Time stats', Icons.insert_chart));
  ...
  void _onMenuItemSelected(AliceMenuItem menuItem) {
    ...
    if (menuItem.title == 'Time stats') {
      _showTimeStatsScreen();
    }
  ...
  void _showTimeStatsScreen() {
    Navigator.push<void>(
      aliceCore.getContext()!,
      MaterialPageRoute(
        builder: (context) => AliceTimeStatsScreen(widget._aliceCore),
      ),
    );
  }
                ...
                return Wrap(
                  children: [
                    ...AliceSortOption.values
                        .map(
                          (sortOption) => RadioListTile<AliceSortOption>(
